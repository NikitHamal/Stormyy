...
async function processKimiStreamingResponse(response, options = { isPreset: false }) {
    ...
    if (response.body) {
        ...
        try {
                            ...
                            try {
                                ...
                                switch (parsed.event) {
                                    ...
                                    case 'all_done':
                                        ...
                                        if (statusElement) {
                                            statusElement.textContent = `Status: ${statusCode} ${response.statusText} (Complete)`;
                                        }
                                        break;
                                }
                            } catch (e) {
            ...
            // Add "Copy Response" button after streaming is complete
            if (messageContent && responseBody) {
                const copyButton = document.createElement('button');
                copyButton.className = 'copy-button';
                copyButton.textContent = 'Copy Response';
                copyButton.onclick = () => copyToClipboard(messageContent, copyButton);
                responseBody.appendChild(copyButton);
            }
